<?php 
    ...
    function recordComment($table)
    {
        if(is_admin()):
            $queryResult = query("select * from ". $table);
        else:
            $queryResult = query("select * from ". $table ." inner join posts on ".$table.".comment_post_id=posts.post_id where posts.user_id={$_SESSION['user_id']}");
        endif;    
        $result = mysqli_num_rows($queryResult);
        if(isset($result))
        {
    ...
    {   
        ...
        while($row = mysqli_fetch_assoc($select_user))
        {
            ...
            if(password_verify($password, $db_user_password))
            {
                ...
                header("location: /cms-theme/admin/");
            }
            else
            {
                return false;
    ...
    function register_user($fname, $lname, $username, $email, $password)
    {
        ...
        $query .= "values ('{$username}', '{$fname}', '{$lname}', '{$email}', '{$password}', 'subscriber')";
        $register_user = mysqli_query($connection, $query);
        confirmQuery($register_user);
    }

    function temp_register_user($fname, $lname, $username, $email, $password)
    {
        global $connection;
        $fname = mysqli_real_escape_string($connection, $fname);
        $lname = mysqli_real_escape_string($connection, $lname);
        ...
        $query = "insert into temp_users (username, user_firstname, user_lastname, user_email, user_password, user_role, otp) ";
        $query .= "values ('{$username}', '{$fname}', '{$lname}', '{$email}', '{$hashed_password}', 'subscriber', '$otp')";
        $register_user = mysqli_query($connection, $query);
        confirmQuery($register_user);
    }
